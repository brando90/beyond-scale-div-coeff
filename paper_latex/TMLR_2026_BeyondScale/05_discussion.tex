...
While our controlled experiments show a consistent positive trend between diversity and evaluation performance across our 44 pre-trained models (Figure~\ref{fig:div_vs_val_ce}), we acknowledge important caveats.
...
Second, none of four downstream accuracy benchmarks---ARC-Easy, HellaSwag, WinoGrande, and LAMBADA---shows a statistically significant correlation with diversity across the 25 evaluated models (all $p>0.18$; Appendix~\ref{appendix:downstream_benchmarks}).
The low absolute accuracies are consistent with a floor effect, but this experiment cannot distinguish that explanation from the absence of a diversity--accuracy relationship at this scale.
We therefore base our diversity$\to$performance claim on the cross-entropy results---a continuous log-likelihood metric of the kind shown to retain capability signal that accuracy's argmax discretization destroys at small scale \citep{schaeffer2024elusive}---while the four accuracy benchmarks provide no supporting evidence in this setting.
Continuous per-choice re-scoring of MMLU across the same models---mean $\log P(\mathrm{correct})$ and its margin over incorrect choices---also shows no significant correlation with diversity (Appendix~\ref{appendix:downstream_benchmarks}).
At this scale, the diversity signal thus appears in corpus-level next-token prediction but not in question-level benchmark metrics, whether discrete or continuous.
