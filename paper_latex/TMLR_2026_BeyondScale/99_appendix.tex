...
We provide an empirical comparison with the Vendi Score in Appendix~\ref{appendix:baseline_metrics}, where the tested GPT-2-kernel configuration produces dataset rankings that contradict both intuition and the diversity coefficient.
...
\label{appendix:token_brittleness}
...
\subsection{Why Shannon Token Entropy is Unsuitable as a Measure of Training-Data Diversity}
\label{appendix:entropy_not_diversity}
A natural question is whether the Shannon entropy $H(p) = -\sum_{x \in V} p(x) \log p(x)$ of a dataset's token (or $n$-gram) distribution suffices as a diversity metric, since it is model-free and cheap to compute.
We argue it does not, for three reasons.

\textbf{Uniform i.i.d.\ noise maximizes token entropy.}
Over a fixed vocabulary $V$, $H$ is uniquely maximized by a uniform marginal token distribution.
Sampling tokens i.i.d.\ uniformly at random produces this marginal and is precisely the synthetic dataset we use as the \emph{conceptual upper bound} in Section~\ref{method_ub_lb}.
It contains no syntax, semantics, or latent concepts, so a language model can learn only the uniform token distribution.
A diversity metric intended to inform data \emph{quality} judgments must therefore not assign its maximal value to data that is worthless for pre-training.
Entropy conflates diversity with randomness; the useful notion for data curation is variety of \emph{learnable structure}.
The diversity coefficient makes this distinction operational: the maximum-entropy dataset serves only as a reference bound (approximately 0.4), and all real, structured corpora we measure fall strictly below it while still ordering sensibly among themselves (Table~\ref{tab:div}).

\textbf{Token entropy is blind to sequence structure.}
Token-level $H$ is a functional of the marginal unigram frequency histogram alone, and is therefore invariant to arbitrary permutations of tokens within and across sequences.
Randomly shuffling every token of C4 destroys its sequence-level syntax and semantics yet leaves its token entropy exactly unchanged.
Extending to $n$-gram entropies only moves this limitation to length-$n$ contexts, whose possible outcomes grow as $|V|^n$, and remains brittle to paraphrasing for the reasons given in Section~\ref{appendix:token_brittleness}.

\textbf{Practical entropy-rate estimation still requires a sequence model.}
The relevant sequence-level quantity is the entropy rate of the text distribution.
For long-range-dependent natural language, practical estimates impose a predictive model or compressor; language-model cross-entropy is one such model-dependent upper bound.
This reintroduces the model dependence that motivates model-free proposals while providing none of the task-structure sensitivity of Fisher Information based embeddings.
Token entropy is additionally tokenizer-dependent, so its absolute values are not comparable across vocabularies.

One alternative applies entropy in embedding space rather than token space: the Vendi Score \citep{friedman2022vendi} is the exponential of the Shannon entropy of the eigenvalues of a similarity kernel.
In Appendix~\ref{appendix:baseline_metrics}, our tested Vendi configuration fails an intuitive dataset-ranking check that the diversity coefficient passes.
...
\section{Empirical Comparison with Simpler Diversity Baselines}
\label{appendix:baseline_metrics}
A reasonable question is whether simpler, cheaper metrics---$n$-gram diversity, embedding distances, or the Vendi Score---already capture what the diversity coefficient measures.
We compare all three families of baselines against the diversity coefficient on six of the pre-training datasets from Table~\ref{tab:div}.

\textbf{Experiments:}
For each of C4, WikiText-103, The Pile, PubMed Abstracts, USPTO, and OpenWebText, we sample 100 batches of 512 sequences using the GPT-2 tokenizer and compute:
(1) \emph{$n$-gram diversity}, the distinct-to-total $n$-gram ratio within batches for $n \in \{1,2,3,4\}$~\citep{li2016diversity};
(2) \emph{mean GPT-2 embedding cosine distance}, the mean pairwise cosine distance between batch vectors, where each batch vector averages L2-normalized final-token embeddings from pretrained GPT-2 \citep{gpt2}; and
(3) the \emph{Vendi Score} \citep{friedman2022vendi}, the exponential of the Shannon entropy of the eigenvalues of the cosine-similarity kernel over the same GPT-2 sequence embeddings, using a fixed random subsample of 500 sequences.
Sentence encoders such as SBERT \citep{reimers2019sentence} provide another possible embedding backbone, but the reported experiment used GPT-2.
Diversity coefficient values are taken from Table~\ref{tab:div} (computed with 200 batches of size 512); because the baseline run used 100 batches, we compare dataset \emph{rankings} rather than absolute magnitudes.

\begin{table}[h!]
\centering
\caption{\textbf{The tested simpler diversity baselines disagree with each other and fail intuitive dataset-ranking checks that the diversity coefficient passes.}
Vendi Score (GPT-2 kernel) ranks the narrow-domain patent corpus USPTO \emph{above} web-scale C4, and mean GPT-2 embedding cosine distance ranks C4 \emph{lowest} of all six datasets---both contradicting the expectation (and the diversity coefficient's finding) that large web-crawl corpora are among the most diverse.
$n$-gram diversity agrees with the diversity coefficient on the extremes but only measures surface lexical variety.
Embedding cosine values are $\times 10^{-4}$.}
\vskip 0.1in
\begin{small}
\begin{tabular}{lccccc}
\toprule
Dataset & Div. Coeff. & 1-gram & 2-gram & GPT-2 cos. ($\times 10^{-4}$) & Vendi \\
\midrule
USPTO & 0.158 & 0.116 & 0.525 & 1.28 & 1.490 \\
PubMed Abstracts & 0.168 & 0.146 & 0.620 & 1.72 & 1.429 \\
WikiText-103 & 0.214 & 0.215 & 0.689 & 2.17 & 1.443 \\
OpenWebText & 0.222 & 0.191 & 0.688 & 1.44 & 1.440 \\
C4 & 0.237 & 0.200 & 0.713 & 1.02 & 1.366 \\
The Pile & 0.246 & 0.191 & 0.664 & 2.48 & 1.603 \\
\bottomrule
\end{tabular}
\end{small}
...
\textbf{Results:}
Table~\ref{tab:baseline_metrics} reports all metrics; our key observations are:
\begin{itemize}
    \item \textbf{$n$-gram diversity broadly agrees with the diversity coefficient} (Spearman $\rho = 0.60$ over the six datasets for both 1-gram and 2-gram): both rank the narrow-domain corpora USPTO and PubMed lowest.
    However, $n$-gram diversity only measures surface lexical variety and is brittle to paraphrasing (Section~\ref{appendix:token_brittleness}).
    \item \textbf{The tested Vendi configuration fails an intuitive dataset-ranking check}: it ranks USPTO (patent applications with highly regular format and topic) \emph{above} the web-scale corpora C4, OpenWebText, and WikiText-103.
    Its rank correlation with the diversity coefficient is near zero ($\rho = 0.03$).
    \item \textbf{Mean GPT-2 embedding cosine distance ranks C4 lowest of all six datasets}, despite C4 being an internet-scale web-crawl corpus that both the diversity coefficient and human intuition place among the most diverse.
    Its rank correlation with the diversity coefficient is weak ($\rho = 0.26$).
    \item \textbf{The simpler baselines also disagree with each other}: 2-gram diversity and the Vendi Score are negatively rank-correlated ($\rho = -0.49$) on these datasets, so they do not induce a common ranking.
\end{itemize}
With only six datasets, these rank correlations are descriptive rather than definitive.
In contrast to the baselines, the diversity coefficient passes the battery of validation checks presented in this paper: it increases under dataset concatenation (Section~\ref{sec:experiments}), correlates with ground-truth diversity on synthetic benchmarks with known diversity (Appendix~\ref{appendix:ground_truth_div} and Section~\ref{body:ginc}), and agrees with GPT-4 diversity judgments at an 82.8\% rate (Section~\ref{sec:gpt4_validation}).
These six-dataset results do not establish universal superiority, but they show that the baseline rankings depend strongly on the representation.
They also suggest that $n$-gram diversity can serve as a cheap lexical proxy, whereas the Task2Vec diversity coefficient has support from the additional validation checks above.
...
\subsection{Downstream Benchmark Evaluations Beyond Cross-Entropy}
\label{appendix:downstream_benchmarks}
To complement the cross-entropy evaluations in Figure~\ref{fig:div_vs_val_ce} and the MMLU results in Section~\ref{sec:confounding}, we evaluate all available checkpoints on four additional standard benchmarks.

\textbf{Experiments:}
We evaluate 25 models (GPT-2 families from 51M to 1.5B parameters and LLaMA-2 7B checkpoints, each pre-trained from scratch on USPTO, PubMed Abstracts, or USPTO+PubMed with diversity coefficients 0.158, 0.168, and 0.195 respectively) on ARC-Easy, HellaSwag, WinoGrande, and LAMBADA using the EleutherAI lm-evaluation-harness.\footnote{Two of the original 27 checkpoints (the 204M and 810M USPTO+PubMed models) were excluded after a pre-submission audit found their eval outputs to be bit-identical duplicates of a USPTO model across every benchmark and MMLU subject---a checkpoint-loading error affecting only those two runs. Excluding them leaves every correlation reported here a non-significant null, as it was before exclusion; details in the released \texttt{experiments/} data-integrity note.}
The reported metric is \emph{accuracy}: length-normalized accuracy (\texttt{acc\_norm}) for ARC-Easy and HellaSwag, and standard accuracy (\texttt{acc}) for WinoGrande and LAMBADA.

\begin{table}[h!]
\centering
\caption{\textbf{Accuracy remains low on four downstream benchmarks, and none shows a significant diversity correlation.}
Mean accuracy is computed over all 25 models; no benchmark shows a statistically significant Spearman correlation between the training-data diversity coefficient and accuracy.}
\vskip 0.1in
\begin{small}
\begin{tabular}{lcccc}
\toprule
Benchmark & Reference floor & Mean acc.\ (min--max) & Spearman $\rho$ & $p$-value \\
\midrule
ARC-Easy & 0.25 & 0.318 (0.255--0.366) & $-0.27$ & 0.19 \\
HellaSwag & 0.25 & 0.272 (0.262--0.286) & $-0.24$ & 0.25 \\
WinoGrande & 0.50 & 0.503 (0.484--0.522) & $0.02$ & 0.94 \\
LAMBADA & $\approx 0$ & 0.039 (0.000--0.095) & $-0.23$ & 0.26 \\
\bottomrule
\end{tabular}
\end{small}
...
\textbf{Results:}
Table~\ref{tab:downstream_benchmarks} shows low absolute accuracy: ARC-Easy averages 0.318 against a 0.25 reference floor, HellaSwag 0.272 against 0.25, WinoGrande 0.503 against 0.50, and LAMBADA 0.039 near its open-vocabulary floor.
No benchmark shows a significant correlation with the diversity coefficient ($p > 0.18$ for all), so these accuracy results do not support a diversity--performance association in this setting.
The low scores and narrow diversity range are consistent with a floor effect, but these data cannot distinguish a floor effect from the absence of a relationship at this scale.
A complementary methodological explanation, which we regard as the more fundamental one, is that accuracy is a \emph{discontinuous} metric: it discretizes the model's continuous per-choice log-likelihoods through an argmax, and such nonlinear metrics are known to mask smooth, continuous improvements in the underlying capability---the ``emergent abilities as a mirage'' effect \citep{schaeffer2023emergent}---and make downstream capability hard to predict at small scale \citep{schaeffer2024elusive}. A null in accuracy at this scale is therefore expected \emph{a priori}, independent of any diversity question; our cross-entropy evaluation is precisely the continuous, un-discretized metric such analyses recommend.
Discretization is not the whole story, however: on MMLU we computed continuous per-choice metrics directly across the same 25 models---mean $\log P(\mathrm{correct\ choice})$ and the margin $\log P(\mathrm{correct}) - \overline{\log P}(\mathrm{incorrect})$ (recomputed from the raw per-sample log-likelihoods over all 57 subjects)---and find no significant Spearman correlation with the diversity coefficient for either metric (mean $\log P$: $\rho=-0.25$, $p=0.24$; margin: $\rho=0.10$, $p=0.62$).
We stress that this continuous null is \emph{consistent with}, not contrary to, the mirage/elusive account, which resolves the two effects along different axes. Those works recover a smooth signal by varying \emph{scale}---model size or training compute---over orders of magnitude, which drives a large, gradually accumulating change in question-level capability; here we instead vary \emph{diversity} over a narrow range (0.158--0.195) at a fixed, sub-threshold scale (tens-to-hundreds of millions of parameters trained on $\leq$2.2B tokens), so there is little graded question-level capability for even a continuous metric to resolve. In other words, a continuous metric removes the argmax artifact but cannot manufacture a capability difference the small models have not acquired. The diversity effect is instead detectable precisely where these models \emph{do} carry graded signal---corpus-level next-token cross-entropy on held-out C4 and OpenWebText2 (Figure~\ref{fig:div_vs_val_ce}), i.e., genuine generalization to text unseen during training---which is why we treat the benchmark results as a scale-limited null rather than as evidence against the hypothesis.

\paragraph{The continuous metric is not powerless: a positive control.}
A natural worry is that the continuous margin simply cannot detect \emph{any} training-data effect at this scale, in which case the null above would be uninformative.
It can. As a pre-registered positive control, we compare the margin of the PubMed-trained versus the USPTO-trained model within each model family on the medicine-related MMLU subjects (clinical knowledge, college medicine, professional medicine, college biology, medical genetics, anatomy).
The PubMed-trained model has the higher margin in \textbf{7 of 7 families} (exact paired sign test $p = 0.0078$), and the effect persists on the full 57-subject pool (6 of 7 families).
The margin therefore reliably distinguishes models by their training corpus; the absence of a \emph{diversity} effect is a substantive statement about scale, not a dead instrument.
Applying the same pre-registered per-family analysis to the diversity contrast (the mix, $\mathrm{div}=0.195$, versus its components) yields no consistent effect: the mix significantly beats USPTO on the HellaSwag margin and the LAMBADA log-likelihood (6/6 families, $p=0.016$ each) but does not significantly beat the intermediate-diversity PubMed model on any benchmark (at most 5/6 families, on LAMBADA, $p=0.11$), while the ARC-Easy and WinoGrande contrasts against USPTO are null---so no benchmark exhibits a significant graded USPTO $<$ PubMed $<$ mix ordering in the diversity coefficient (details in \texttt{experiments/07\_matched\_subset\_margin}).

At these model scales and token budgets, the benefit of pre-training data diversity is therefore detectable in corpus-level cross-entropy but not in question-level benchmark metrics, discrete or continuous.
FineWeb \citep{penedo2024fineweb} and DataComp-LM \citep{li2024datacomp} likewise select benchmark subsets designed to provide reliable signal in small-scale data-ablation experiments.
Our main diversity$\to$performance evidence therefore rests on the cross-entropy results in Figure~\ref{fig:div_vs_val_ce}; the four accuracy benchmarks reported here provide no supporting evidence.
Testing whether larger training budgets or early-signal benchmark subsets reveal an accuracy relationship remains future work.
...
\begin{itemize}
    ...
    Alternatives such as the Vendi score are promising as aggregation functions over Task2Vec distances, but still under-explored and computationally demanding compared to expectations/sums.
    Future work could focus on the comparative analysis of the total distance sum and the Vendi score as aggregators over Task2Vec distances (Appendix~\ref{appendix:baseline_metrics} already compares the Vendi Score computed over raw GPT-2 embeddings as a standalone baseline metric).
