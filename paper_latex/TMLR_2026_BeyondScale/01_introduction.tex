...
\begin{enumerate}
    ...
    \item We provide evidence via \textbf{interventional} experiments with various GPT-2 and LLaMAv2 models that pre-training models on data with higher diversity coefficients leads to better downstream performance on language modeling for diverse evaluation corpora---totaling 44 models pre-trained from scratch of various sizes (51M to 7B).\footnote{Model counts differ by scope throughout the paper: \textbf{44} is the total number of from-scratch pre-training runs across all sizes, token budgets, and repeated LLaMA-2 training seeds; the cross-entropy analysis in Figure~\ref{fig:div_vs_val_ce} visualizes these runs grouped by configuration; and the downstream benchmark and MMLU analyses (Appendix~\ref{appendix:downstream_benchmarks}) cover \textbf{25} distinct checkpoints (\textbf{27} before excluding two with corrupted eval outputs). The counts differ because repeated LLaMA-2 runs are counted individually for cross-entropy but map to distinct released checkpoints for the benchmark analysis; the released experiment logs give the exact census.}
