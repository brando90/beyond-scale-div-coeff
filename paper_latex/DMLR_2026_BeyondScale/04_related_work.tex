...
While elegant, it requires materializing an $N\times N$ kernel and eigendecomposing it, which limits scalability to web-scale corpora; we provide a detailed comparison in Appendix~\ref{related_work_cont}.
Simpler baselines such as N-gram diversity (distinct-to-total N-gram ratio)~\citep{li2016diversity} and mean embedding cosine distance are computationally cheap but do not capture structural task-level diversity; we compare against these baselines empirically in Appendix~\ref{appendix:baseline_metrics}.
...
Subsequent work shows that the measured diversity of synthetic pre-training data predicts downstream LLM performance~\citep{chen2024diversity}, corroborating diversity as a key axis of data quality.
