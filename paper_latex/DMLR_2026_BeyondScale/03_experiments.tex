...
On MMLU, the average accuracy across model families is essentially flat and within noise of the 0.25 chance floor---USPTO (0.234), PubMed (0.236), USPTO+PubMed (0.236)---so, consistent with Appendix~\ref{appendix:downstream_benchmarks}, we do not read these accuracy values as evidence either for or against the diversity ordering; the domain-control argument below rests on cross-entropy, not accuracy.
Since no training domain matches the evaluation domain, the cross-entropy improvement is attributable to diversity rather than domain overlap.
