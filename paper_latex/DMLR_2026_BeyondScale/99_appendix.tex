...
Mean accuracy is computed over all 27 models; no benchmark shows a statistically significant Spearman correlation between the training-data diversity coefficient and accuracy.}
\vskip 0.1in
\begin{small}
\begin{tabular}{lcccc}
\toprule
...
\midrule
ARC-Easy & 0.25 & 0.320 (0.255--0.366) & $-0.16$ & 0.41 \\
HellaSwag & 0.25 & 0.272 (0.262--0.286) & $-0.14$ & 0.49 \\
WinoGrande & 0.50 & 0.504 (0.484--0.522) & $0.10$ & 0.62 \\
LAMBADA & $\approx 0$ & 0.043 (0.000--0.095) & $-0.08$ & 0.68 \\
\bottomrule
\end{tabular}
\end{small}
...
\textbf{Results:}
Table~\ref{tab:downstream_benchmarks} shows low absolute accuracy: ARC-Easy averages 0.320 against a 0.25 reference floor, HellaSwag 0.272 against 0.25, WinoGrande 0.504 against 0.50, and LAMBADA 0.043 near its open-vocabulary floor.
No benchmark shows a significant correlation with the diversity coefficient ($p > 0.4$ for all), so these accuracy results do not support a diversity--performance association in this setting.
...
% TODO(internal-review): EXPERIMENT_STATE.md §6 flags one duplicated eval-results row in mmlu_results.csv (gpt2-810M-2.2B-USPTOAndPubMedAbs identical to gpt2-117M-2.2B-USPTO, likely a results-dir casing collision). Sensitivity check 2026-07-21: dropping the suspect rows leaves both correlations null (log_p_correct rho in [-0.22,-0.17], p>=0.27; margin |rho|<=0.05, p>=0.83 across n=27/25/23 variants), so the claim below is robust — but fix the CSV before camera-ready and update the two rho/p values if they shift.
Discretization is not the whole story, however: on MMLU we computed continuous per-choice metrics directly across the same 27 models---mean $\log P(\mathrm{correct\ choice})$ and the margin $\log P(\mathrm{correct}) - \overline{\log P}(\mathrm{incorrect})$---and find no significant Spearman correlation with the diversity coefficient for either metric (mean $\log P$: $\rho=-0.17$, $p=0.39$; margin: $\rho=0.04$, $p=0.83$).
At these model scales and token budgets, the benefit of pre-training data diversity is therefore detectable in corpus-level cross-entropy but not in question-level benchmark metrics, discrete or continuous; extending the per-choice log-likelihood analysis to the four benchmarks above remains future work.
